...
ecosumstats <- function(y) {
  ...
  if(is.matrix(y)) {
    ...
    c.score <- c_score(ifelse(y > 0, 1, 0))
    c.score <- c_score(ifelse(y > 0, 1, 0))
    c.score.skew <- c_score_skew(ifelse(y > 0, 1, 0))
    c.score.var <- c_score_var(ifelse(y > 0, 1, 0))
    checkerscore <- checker(ifelse(y > 0, 1, 0))
    ...
    c.score <- sapply(incs, function(x) c_score(x))
    c.score.skew <- sapply(incs, function(x) c_score_skew(x))
    c.score.var <- sapply(incs, function(x) c_score_var(x))
    checkerscore <- sapply(incs, function(x) checker(x))
